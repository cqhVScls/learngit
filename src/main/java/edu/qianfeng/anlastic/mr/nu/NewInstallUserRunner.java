...
public class NewInstallUserRunner implements Tool {
    ...
    public static void main(String[] args) {
        try {
            int isok = ToolRunner.run(new Configuration(), new NewInstallUserRunner(), args);
            System.exit(isok);
        } catch (Exception e) {
            e.printStackTrace();
        }
...
//        return job.waitForCompletion(true)?0:1;
        if (job.waitForCompletion(true)) {
            this.calculateTotalUser(job);  //计算新增总用户
            return 0;
        } else {
            return 1;
        }
     ...
     * @param job
     */
    private void calculateTotalUser(Job job) {
        //获取统计日前一天的所有维度的总用户数据；获取统计当日的新增用户数据；
        // 将统计当日的新增数据和前一天的相同维度数据相加，然后更新即可
        ...
        try {
            ...
            if (nowDateDimensionId > 0) {
                ...
                while (rs.next()) {
                    int platformId = rs.getInt("platform_dimension_id");
                    int totalUser = rs.getInt("new_install_users");
            ...
            //更新用户模块新增总用户数据
            ps = conn.prepareStatement("insert into `stats_device_browser`(`date_dimension_id`,`platform_dimension_id`,`browser_dimension_id`,`total_install_users`) values(?,?,?,?) on duplicate key update `total_install_users` = ?");
            //循环map
            for (Map.Entry<String, Integer> entry : map.entrySet()) {
                ps.setInt(1, nowDateDimensionId);
    ...
    private List<Scan> listScans(Job job) {
        ...
        fl.addFilter(new SingleColumnValueFilter(
                Bytes.toBytes(EventLogConstant.LOG_FAMILY_NAME),
                Bytes.toBytes(EventLogConstant.LOG_COLUMN_NAME_EVENT_NAME),
                CompareFilter.CompareOp.EQUAL,
                Bytes.toBytes(EventLogConstant.EventEnum.LAUNCH.alias)));
        ...
        String columns[] = new String[]{
                ...
                EventLogConstant.LOG_COLUMN_NAME_EVENT_NAME,
