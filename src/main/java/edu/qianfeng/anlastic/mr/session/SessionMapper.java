...
public class SessionMapper extends TableMapper<StatsUserDimension, TimeOutputValue> {
    ...
    protected void map(ImmutableBytesWritable key, Result value, Context context) throws IOException, InterruptedException {
        ...
        //判断该指标的必须字段不能为空
        if (StringUtils.isBlank(serverTime) || StringUtils.isBlank(sessionId) || StringUtils.isBlank(plaform)) {
            logger.warn("serverTime&&sessionId&&plaform三者任意一个都不能为空:serverTime:" + serverTime + " sessionId:" + sessionId + " platform:" + plaform);
            return;
        }
        //代码到这儿正常处理
        ...
        this.v.setTime(longOfTime);  //一定要设置，用于reduce阶段统计会话长度
