...
public class SessionRunner implements Tool{
    ...
    public static void main(String[] args) {
        try {
            int isok = ToolRunner.run(new Configuration(),new SessionRunner(),args);
            System.exit(isok);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public int run(String[] args) throws Exception {
        ...
        //获取job
        Job job = Job.getInstance(conf,"session runner");
        job.setJarByClass(SessionRunner.class);

        //初始化TableMapper  本地提交本地运行
        TableMapReduceUtil.initTableMapperJob(this.listScans(job),SessionMapper.class,
...
//                StatsUserDimension.class,TimeOutputValue.class,job,true);
    ...
    private List<Scan> listScans(Job job) {
        ...
        String columns[] = new String[]{
                ...
                EventLogConstant.LOG_COLUMN_NAME_SESSION_ID,
