...
public class PageViewRunner implements Tool{
    ...
    public static void main(String[] args) {
        try {
            int isok = ToolRunner.run(new Configuration(),new PageViewRunner(),args);
            System.exit(isok);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public int run(String[] args) throws Exception {
        ...
        TableMapReduceUtil.initTableMapperJob(this.listScans(job),PageViewMapper.class,
...
//                StatsUserDimension.class,TimeOutputValue.class,job,true);
    ...
    private List<Scan> listScans(Job job) {
        ...
        fl.addFilter(new SingleColumnValueFilter(
                ...
                Bytes.toBytes(EventLogConstant.EventEnum.PAGEVIEW.alias)));

        //设置需要扫描出来的字段
        String columns[] = new String[]{
                EventLogConstant.LOG_COLUMN_NAME_SERVER_TIME,
                EventLogConstant.LOG_COLUMN_NAME_CURRENT_URL,
